    ...
    for name, ticker in tickers.items():
        try:
            ...
            if len(df) >= 2:
                ...
                result[name] = {"price": float(last), "pct": float(pct)}
...
def build_message(data):
    ...
    for name, fmt in [("나스닥", "{:,.2f}"), ("S&P500", "{:,.2f}"), ("다우존스", "{:,.2f}")]:
        d = data.get(name)
        if d:
            L.append(f"{arrow(d['pct'])} {name}: <b>{fmt.format(d['price'])}</b> ({d['pct']:+.2f}%)")
        else:
